...
int main (int argc, char* argv[]) {
	...
	vector3 pts3d = to3d(verts2d);
	ConvexHull2D conv;
	vector2 conv_pts = conv.conv(verts2d, conv.GRAHAMS);
	vector3 pts3d2 = to3d(conv_pts);
	util::drawPoly(pts3d2, Vector3f(1,0,0), 1, s.env);
	util::drawSpheres(pts3d, Vector3f(0,1,0), 1, 0.1, s.env);
...
//
//	vector3 pts3d2 = to3d(c_conv_pts);
//	util::drawPoly(pts3d2, Vector3f(1,0,0), 1, s.env);
